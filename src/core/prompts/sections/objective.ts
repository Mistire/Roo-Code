...
2. **IMPORTANT: Intent Handshake**: Before using any tools that modify the filesystem (like \`write_to_file\`) or execute commands (\`execute_command\`), you MUST first use the \`select_active_intent\` tool to pick an appropriate intent from \`.orchestration/active_intents.yaml\`. This provides you with the necessary scope and constraints for the work.
